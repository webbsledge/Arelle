...
def validateXbrlFinally(val: ValidateXbrl, *args: Any, **kwargs: Any) -> None:
    ...
    if modelDocument.type in (ModelDocument.Type.INLINEXBRL, ModelDocument.Type.INLINEXBRLDOCUMENTSET, ModelDocument.Type.INSTANCE, ModelDocument.Type.UnknownXML):
        ...
        contextData = checkContexts(modelXbrl, esefYear=esefDisclosureSystemYear)
        contextIdentifiers = defaultdict(list)
        for context in modelXbrl.contexts.values():
            contextIdentifiers[context.entityIdentifier].append(context)

        if contextData.contextsWithSegments:
            modelXbrl.error("ESEF.2.1.3.segmentUsed",
                _("xbrli:segment container MUST NOT be used in contexts: %(contextIds)s"),
                modelObject=contextData.contextsWithSegments, contextIds=", ".join(c.id for c in contextData.contextsWithSegments if c.id is not None))
        if contextData.contextsWithImproperContent:
            modelXbrl.error("ESEF.2.1.3.scenarioContainsNonDimensionalContent",
                ...
                modelObject=contextData.contextsWithImproperContent, contextIds=", ".join(c.id for c in contextData.contextsWithImproperContent if c.id is not None))
            ...
            if contextScheme != requiredScheme:
                ...
                if leiValidity == LeiUtil.LEI_INVALID_LEXICAL:
                    ...
                    modelXbrl.error("ESEF.2.1.1.invalidIdentifier",
                        ...
                        modelObject=contextElts, identifier=contextIdentifier)
        if contextData.contextsWithPeriodTime:
            modelXbrl.error("ESEF.2.1.2.periodWithTimeContent",
                ...
                modelObject=contextData.contextsWithPeriodTime, contextIds=", ".join(c.id for c in contextData.contextsWithPeriodTime if c.id))
        if contextData.contextsWithPeriodTimeZone:
            modelXbrl.error("ESEF.2.1.2.periodWithTimeZone",
                ...
                modelObject=contextData.contextsWithPeriodTimeZone, contextIds=", ".join(c.id for c in contextData.contextsWithPeriodTimeZone if c.id))
        if contextData.contextsWithWrongInstantDate:
            for context in contextData.contextsWithWrongInstantDate:
