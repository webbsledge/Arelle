...
def parseArgs(args: list[str]) -> tuple[RuntimeOptions, dict]:
    ...
    validationGroup.add_option(
        ...
        default=ValidateDuplicateFacts.DeduplicationType.COMPLETE.value,
        help=_(
            "When using `--saveDeduplicatedInstance` to save a deduplicated instance, check for duplicates of this type"
            " (`complete`, `consistent-pairs`, `consistent-sets`). Defaults to `complete`."
            )
        )
    validationGroup.add_option(
    ...
    for featureName, featureDescription in BETA_FEATURES_AND_DESCRIPTIONS.items():
        ...
        betaGroup.add_option(
            f"--{featureName}", f"--{featureName.lower()}", action="store_true", default=False, help=featureDescription
            )
    ...
    outputFileGroup.add_option(
        ...
        action="store",
        dest="roleTypesFile",
        help=_("Store defined role types in specified `FILE`")
        )
    outputFileGroup.add_option(
        "--arcroleTypes",
        "--arcroletypes",
        ...
        help=_("Store defined arcrole types in specified `FILE`")
        )
    outputFileGroup.add_option(
        "--testReport",
        "--csvTestReport",
    ...
    loggingGroup.add_option(
        ...
        help=_(
            ...
            "The default is 4096000 for JSON content and 128 for everything else."
            )
        )
    loggingGroup.add_option(
        "--collectProfileStats",
    ...
    formulaGroup.add_option(
        ...
        help=_(
            ...
            "Negative numbers have no limit. (10_000_000 is default)"
            )
        )
    formulaGroup.add_option(
    ...
    configGroup.add_option(
        ...
        help=_(
            ...
            "`hourly`, `daily`, `weekly` or `never` (`weekly` is default)"
            )
        )
    configGroup.add_option(
