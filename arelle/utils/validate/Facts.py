...
def factHasNegativeNumericValue(fact: ModelFact) -> bool:
    ...
    """
    return fact.xValid >= VALID and fact.xValue is not None and cast(int, fact.xValue) < 0
...
def getRequiredFactMissing(
    modelXbrl: ModelXbrl,
    conceptKey: Union[QName, str],
    byQname: bool = True,
) -> bool:
    """
    Returns True if no fact with the given concept key is tagged with a non-nil value.
    conceptKey is QName when byQname=True, else str (localName).
    """
    if byQname:
        facts = modelXbrl.factsByQname.get(cast(QName, conceptKey), set())
    else:
        facts = modelXbrl.factsByLocalName.get(cast(str, conceptKey), set())
    for fact in facts:
        if not fact.isNil:
            return False
    return True
